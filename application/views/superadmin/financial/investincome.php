<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	if($_REQUEST['from_date']!='' && $_REQUEST['to_date']!=''){
		...
		$StrWhr .=" AND DATE(tmi.date_time) BETWEEN '$from_date' AND '$to_date'";
		$SrchQ .="&from_date=$from_date&to_date=$to_date";
	}
	
	if($_REQUEST['user_id']!=''){
		$member_id = $model->getMemberId($_REQUEST['user_id']);
		$StrWhr .=" AND tmi.member_id='$member_id'";
		$SrchQ .="&user_id=$user_id";
	}

	
	$QR_PAGES="SELECT tmi.*, tm.user_id FROM tbl_mem_invest AS tmi 
			   LEFT JOIN tbl_members AS tm ON tmi.member_id=tm.member_id
    ...
    <div class="main-content-inner">
      ...
      <div class="page-content">
        <div class="page-header">
          <h1> Financial <small> <i class="ace-icon fa fa-angle-double-right"></i> &nbsp;Re-investment Income </small> </h1>
        </div>
        <!-- /.page-header -->
        <div class="row">
          <?php get_message(); ?>
