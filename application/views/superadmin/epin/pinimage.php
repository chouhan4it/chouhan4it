...
$QR_PAGES= "SELECT tpi.* FROM tbl_pin_image AS tpi
			WHERE tpi.delete_sts>0 AND tpi.type_id='".$type_id."'
		   ORDER BY tpi.pin_img_id DESC";
    ...
    <div class="main-content-inner">
      ...
      <div class="page-content">
        <div class="page-header">
          <h1> Pakcage Image <small> <i class="ace-icon fa fa-angle-double-right"></i> &nbsp; <?php echo $AR_PIN['pin_name'];  ?> </small> </h1>
        </div>
        <!-- /.page-header -->
        <div class="row">
          <?php get_message(); ?>
                  ...
                  <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
			 	 	...
			 	 	if($PageVal['TotalRecords'] > 0){
						...
						$IMG_SRC = $model->getPinFileSrc($AR_DT['pin_img_id']);
			       ?>
                      <tr>
                        <td class="center"><?php echo $Ctrl; ?></td>
                        <td><a target="_blank" href="<?php echo $IMG_SRC; ?>"> <img src="<?php echo $IMG_SRC; ?>" width="60px" height="60px" class="img-responsive"></a></td>
                        <td><?php echo $AR_DT['file_type']; ?></td>
                        <td><?php echo $AR_DT['file_size']; ?> KB</td>
                        <td><input type="radio" name="cover_sts" id="cover_sts" class="cover_sts" pin_img_id="<?php echo $AR_DT['pin_img_id']; ?>" type_id="<?php echo $AR_DT['type_id']; ?>" <?php echo checkRadio($AR_DT['cover_sts'],"1") ?>></td>
                        <td><div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-default">
                              <li> <a onClick="return confirm('Make sure , you want to delete this document?')" href="<?php echo generateSeoUrlAdmin("epin","pinimage",array("pin_img_id"=>_e($AR_DT['pin_img_id']),"action_request"=>"DELETE")); ?>">Delete</a> </li>
                            </ul>
                          </div></td>
                      </tr>
  ...
  <div id="search-modal" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form class="form-horizontal"  name="form-page" id="form-page" action="<?php echo generateAdminForm("epin","pinimage","");  ?>" enctype="multipart/form-data" method="post">
          <div class="modal-body">
          <div class="form-group">
            <label class="col-sm-3 control-label no-padding-right" for="form-field-1-1"> Photo :</label>
            <div class="col-sm-7">
              <input type="file" name="pin_image[]" id="pin_image" multiple>
            </div>
          </div>
          <div class="modal-footer">
            <button type="submit" name="submit-image" value="1" class="btn btn-sm btn-success"> <i class="ace-icon fa fa-check"></i> Submit </button>
            &nbsp;
            <input type="hidden" name="action_request" id="action_request"  value="ADD_UPDATE">
            <input type="hidden" name="type_id" id="type_id" value="<?php echo _e($type_id); ?>">
...
<script type="text/javascript">
	$(function(){
		...
		$(".cover_sts").on('click',function(){
			var pin_img_id = $(this).attr("pin_img_id");
			var type_id = $(this).attr("type_id");
			if(pin_img_id>0){
				var URL_LOAD  = "<?php echo generateSeoUrl("json","jsonhandler",""); ?>";
				$.getJSON(URL_LOAD,{switch_type:"PIN_COVER",pin_img_id:pin_img_id,type_id:type_id},function(JsonEval){
					if(JsonEval){
